          ...
          {checklistEntries.map((entry, idx) => (
            <div key={entry.checklistItemId} className={`rounded-lg border p-2.5 ${entry.url.trim() ? 'border-green-200 bg-green-50/50' : 'border-gray-200 bg-white'}`}>
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <span className="text-xs font-semibold text-gray-800 flex-1">{entry.title}</span>
                <select
                  value={entry.labelId}
              ...
              <div className="ml-6 space-y-1.5">
                ...
                {entry.isUploaded ? (
                  /* Uploaded state - show Drive link */
                  <div className="flex items-center gap-1.5 bg-green-50 border border-green-200 rounded-lg px-2.5 py-2">
                    <svg className="w-4 h-4 text-green-600 shrink-0" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M7.71 3.5L1.15 15l4.58 7.5h13.54l4.58-7.5L17.29 3.5H7.71zm-.44 1h4.06L6.1 14H2.04L7.27 4.5zm5.06 0h4.06l5.23 9.5h-4.06L12.33 4.5zM6.73 5.89L11.84 15H2.62l4.11-9.11zM12.16 5.89L17.27 15H8.05l4.11-9.11zm-5.6 10.11h10.88l-2.72 4.5H9.28L6.56 16z" />
                    </svg>
                    <a
                      href={entry.url}
                      target="_blank"
                  ...
                  /* Uploading state */
                  <div className="flex items-center justify-center gap-2 border-2 border-dashed border-blue-300 bg-blue-50 rounded-lg py-4">
                    <svg className="w-5 h-5 animate-spin text-blue-500" fill="none" viewBox="0 0 24 24">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                    </svg>
                    <span className="text-xs text-blue-600 font-medium">Đang upload lên Google Drive...</span>
                  </div>
                ) : (
                  /* Default: drag & drop zone + manual link */
                  <div className="space-y-1.5">
                    <div
                      className={`border-2 border-dashed rounded-lg py-3 px-3 text-center cursor-pointer transition-all ${
                        draggingIdx === idx
                      ...
                      onDragEnter={e => { e.preventDefault(); setDraggingIdx(idx); }}
                      onDragLeave={() => setDraggingIdx(null)}
                      onDrop={e => {
                        e.preventDefault();
                        setDraggingIdx(null);
                        const fl = e.dataTransfer.files;
                        if (fl.length > 0) handleUploadToDrive(idx, fl);
                      }}
                    >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] text-gray-400">hoặc nhập link</span>
                      <div className="flex-1 h-px bg-gray-200" />
                    </div>
                    <input
                      type="url"
